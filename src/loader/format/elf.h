...
struct Elf_header
{
    ...
    elf32_addr e_entry;     // Virtual address to which the system first transfers control, thus starting the process. Zero if the file has no associated entry point 
    elf32_off e_phoff;
    elf32_off e_shoff;
    elf32_word e_flags;
    elf32_half e_ehsize;
    elf32_half e_phentsize;
    elf32_half e_phnum;     // Number of entries in the program header table. Product of e_phentsize and e_phnum gives the table's size in bytes. e_phnum is zero if a file has no program header table, 
